...
const TEST_FILE_SERVER = process.env.TEST_FILE_SERVER || 'http://localhost:8888';
...
test.describe('LANbu Handy MVP User Stories', () => {
  ...
  test('US001: Submit Model URL - Valid .3mf file', async ({ page }) => {
    ...
    await page.fill('[data-testid="model-url-input"]', 
    ...
    await expect(page.locator('[data-testid="status-message"]')).toContainText('Model processed successfully');
    
    // Verify filament requirements are displayed
    await expect(page.locator('[data-testid="filament-requirements"]')).toBeVisible();
  });

  test('US001: Submit Model URL - Invalid URL handling', async ({ page }) => {
    // Test error handling with invalid URL
    await page.fill('[data-testid="model-url-input"]', 'http://invalid-url.example/nonexistent.3mf');
    ...
    await expect(page.locator('[data-testid="error-message"]')).toContainText('Failed to download');
  });

  test('US002: Printer Selection - Display configured printer', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="printer-selector"]')).toBeVisible();
    
    // Check if mock printer is displayed
    await expect(page.locator('[data-testid="printer-option"]')).toBeVisible();
  });

  test('US003: View Model Filament Needs - Multi-color model', async ({ page }) => {
    ...
    const filamentItems = page.locator('[data-testid="filament-requirement-item"]');
  ...
  test('US004: View AMS Filaments - Status display', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="ams-status-section"]')).toBeVisible();
    
    // Check for AMS refresh button
    await expect(page.locator('[data-testid="ams-refresh-button"]')).toBeVisible();
  });

  test('US005-US006: Filament Mapping - Auto and Manual Assignment', async ({ page }) => {
    // Submit multi-color model
    await page.fill('[data-testid="model-url-input"]', 
      `${TEST_FILE_SERVER}/multicolor-test-coin.3mf`);
    ...
    const mappingDropdowns = page.locator('[data-testid="filament-mapping-dropdown"]');
  ...
  test('US007: Select Build Plate - Options and selection', async ({ page }) => {
    // Submit model first
    await page.fill('[data-testid="model-url-input"]', 
      `${TEST_FILE_SERVER}/Original3DBenchy3Dprintconceptsnormel.3mf`);
    ...
    await expect(page.locator('[data-testid="build-plate-selector"]')).toBeVisible();
    
    // Test selecting different build plate
    await page.selectOption('[data-testid="build-plate-selector"]', 'textured_pei');
    
    // Verify selection is applied
    await expect(page.locator('[data-testid="build-plate-selector"]')).toHaveValue('textured_pei');
  });

  test('US009-US010: Initiate Slicing with Progress Feedback', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="slicing-progress"]')).toBeVisible();
  ...
  test('US011-US012: Initiate Print with Feedback', async ({ page }) => {
    ...
    // Wait and slice
    await page.waitForSelector('[data-testid="slice-button"]', { timeout: 30000 });
    await page.click('[data-testid="slice-button"]');
    
    // Wait for slicing completion
    await page.waitForSelector('[data-testid="print-button"]', { timeout: 120000 });
    ...
    const statusText = await page.locator('[data-testid="print-status"]').textContent();
    expect(statusText).toMatch(/(started|submitted|error|failed)/i);
  });

  test('US013: Clear Error Handling - Network errors', async ({ page }) => {
    // Test with unreachable URL
    await page.fill('[data-testid="model-url-input"]', 'http://unreachable.local/model.3mf');
  ...
  test('US014: PWA Access and Functionality', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="analyze-model-button"]')).toBeVisible();
  });
  ...
  test('Responsive Design - Mobile viewport', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="analyze-model-button"]')).toHaveCSS('min-height', /48px|3rem/);
  ...
  test('Workflow Reset - New Model functionality', async ({ page }) => {
    ...
    // Wait for analysis
    await page.waitForSelector('[data-testid="model-analysis-success"]', { timeout: 30000 });
    ...
    await expect(page.locator('[data-testid="filament-requirements"]')).not.toBeVisible();
    await expect(page.locator('[data-testid="ams-status-section"]')).not.toBeVisible();
  });
...
test.describe('Browser Compatibility Tests', () => {
  test('Essential features work across browsers', async ({ page, browserName }) => {
    ...
    await expect(page.locator('[data-testid="model-url-input"]')).toBeVisible();
    await expect(page.locator('[data-testid="analyze-model-button"]')).toBeEnabled();
    
    // Test form interaction
    await page.fill('[data-testid="model-url-input"]', 'https://example.com/test.3mf');
    const inputValue = await page.locator('[data-testid="model-url-input"]').inputValue();
