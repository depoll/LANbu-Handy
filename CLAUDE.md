...
- use `scripts/start-dev.sh` to start the dev servers. These will track changes and restart/refresh when code changes. You will want to run this in the background as it stays running until you ctrl-c it to shut it down.
- When you can safely run tasks in parallel, you should.
- When possible, use the playwright mcp server rather than command line commands to test things in the browser.
