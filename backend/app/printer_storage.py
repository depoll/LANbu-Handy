...
class PrinterStorage:
    ...
    def load_printers(self) -> List[PrinterConfig]:
        ...
        if not self.config_file.exists():
            logger.info("No persistent printer config file found - returning empty list")
            return []
    ...
    def add_printer(self, printer: PrinterConfig) -> None:
        ...
        for existing_printer in printers:
            if existing_printer.ip == printer.ip:
                raise PrinterStorageError(f"Printer with IP {printer.ip} already exists")
    ...
    def update_printer(self, ip: str, name: Optional[str] = None, 
