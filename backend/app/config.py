...
class Config:
                    ...
                    for i, printer_data in enumerate(printers_data):
                        try:
                            if not isinstance(printer_data, dict):
                                logger.error(f"Printer {i} must be a JSON object")
                                continue
                            ...
                            printer = PrinterConfig(name=name, ip=ip, access_code=access_code)
                            printers.append(printer)
                            logger.info(f"Configured printer: {printer.name} at {printer.ip}")
                            
                        except (ValueError, KeyError) as e:
                            logger.error(f"Invalid configuration for printer {i}: {e}")
                            continue
        ...
        if legacy_ip:
            ...
            if legacy_ip:
                ...
                if not legacy_access_code.strip():
                    logger.warning("BAMBU_PRINTER_IP is set but BAMBU_PRINTER_ACCESS_CODE is missing. "
                                 "Please provide access code or use BAMBU_PRINTERS format.")
                    return []
                ...
                try:
                    ...
                    printers.append(printer)
                    logger.info(f"Configured legacy printer: {printer.name} at {printer.ip}")
                    return printers
                except ValueError as e:
                    logger.error(f"Invalid legacy printer configuration: {e}")
        
        # No printers configured
        logger.warning("No printer configuration found. Set BAMBU_PRINTERS (JSON format) "
                      "or BAMBU_PRINTER_IP + BAMBU_PRINTER_ACCESS_CODE for legacy format. "
    ...
    def get_default_printer(self) -> Optional[PrinterConfig]:
        ...
        Returns:
            PrinterConfig: The first printer configuration if any exists, None otherwise
        """
    ...
    def get_printer_ip(self) -> Optional[str]:
        ...
        Returns:
            str: The first printer's IP address if configured, None otherwise
        """
