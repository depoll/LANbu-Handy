...
class TestPrinterManagementIntegration:
    ...
    def test_config_loads_persistent_printers(self):
        ...
        printer = PrinterConfig(
            name="Stored Printer", ip="192.168.1.100", access_code="123"
        )
    ...
    def test_config_update_persistent_printer(self):
        ...
        # Update it
        updated = config.update_persistent_printer(
    ...
    def test_config_active_printer_cleared_when_removed(self):
        ...
        printer = PrinterConfig(
            name="Active Printer", ip="192.168.1.100", access_code="123"
        )
        config.add_persistent_printer(printer)
    ...
    def test_config_mixed_environment_and_persistent_printers(self):
        ...
        with patch.dict(
            "os.environ",
            {
                "PRINTER_CONFIG_FILE": str(self.config_file),
                "BAMBU_PRINTERS": (
                    '[{"name":"Env Printer","ip":"192.168.1.200",'
                    '"access_code":"env123"}]'
                ),
            },
        ):
            ...
            # Add a persistent printer
            persistent_printer = PrinterConfig(
                name="Persistent Printer", ip="192.168.1.100", access_code="pers123"
            )
    ...
    def test_config_persistent_printers_override_environment_same_ip(self):
        ...
        with patch.dict(
            "os.environ",
            {
                "PRINTER_CONFIG_FILE": str(self.config_file),
                "BAMBU_PRINTERS": (
                    '[{"name":"Env Printer","ip":"192.168.1.100",'
                    '"access_code":"env123"}]'
                ),
            },
        ):
