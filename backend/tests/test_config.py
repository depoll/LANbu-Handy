...
class TestPrinterConfig:
    ...
    def test_printer_config_valid(self):
        ...
        printer = PrinterConfig(name="Test Printer", ip="192.168.1.100", access_code="12345678")
...
class TestConfig:
    ...
    def test_config_with_bambu_printers_json_single(self):
        ...
        printers_json = json.dumps([{"name": "Test Printer", "ip": "192.168.1.100", "access_code": "12345678"}])
    ...
    def test_config_with_bambu_printers_json_multiple(self):
        ...
        printers_json = json.dumps([
            {"name": "Living Room", "ip": "192.168.1.100", "access_code": "12345678"},
    ...
    # Test legacy format
    @patch.dict(os.environ, {'BAMBU_PRINTER_IP': '192.168.1.100', 'BAMBU_PRINTER_ACCESS_CODE': 'test123'})
    ...
    @patch.dict(os.environ, {'BAMBU_PRINTER_IP': '  192.168.1.200  ', 'BAMBU_PRINTER_ACCESS_CODE': '  test456  '})
    ...
    def test_config_with_legacy_printer_ip_missing_access_code(self):
        """Test config when legacy BAMBU_PRINTER_IP is set but access code is missing."""
        config = Config()
    ...
    def test_config_logging_when_json_printers_configured(self, caplog):
        """Test that info messages are logged when printers are configured via JSON."""
        printers_json = json.dumps([{"name": "Test Printer", "ip": "192.168.1.50", "access_code": "12345678"}])
    ...
    @patch.dict(os.environ, {'BAMBU_PRINTER_IP': '192.168.1.50', 'BAMBU_PRINTER_ACCESS_CODE': 'test123'})
    def test_config_logging_when_legacy_ip_set(self, caplog):
        """Test that info message is logged when legacy IP is set."""
        with caplog.at_level("INFO"):
            Config()
        
        assert "Configured legacy printer: Default Printer at 192.168.1.50" in caplog.text

    @patch.dict(os.environ, {'BAMBU_PRINTER_IP': '192.168.1.100'})
    def test_config_logging_when_access_code_missing(self, caplog):
        """Test that warning is logged when access code is missing for legacy format."""
        with caplog.at_level("WARNING"):
            Config()
        
        assert "BAMBU_PRINTER_IP is set but BAMBU_PRINTER_ACCESS_CODE is missing" in caplog.text

    @patch.dict(os.environ, {}, clear=True)
    def test_config_logging_when_no_printers_configured(self, caplog):
        """Test that warning message is logged when no printers are configured."""
        with caplog.at_level("WARNING"):
            Config()
