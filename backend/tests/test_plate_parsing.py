...
class TestPlateParsing(unittest.TestCase):
    ...
    def test_parse_stl_file(self):
        ...
        # Write a minimal binary STL file with one triangle
        import struct

        with open(stl_path, "wb") as f:
            # STL header (80 bytes)
            f.write(b"Generated by test" + b"\x00" * 63)
            # Number of triangles (4 bytes, little endian)
            f.write(struct.pack("<I", 1))
            # One triangle (50 bytes)
        ...
        try:
            ...
            model_info, _ = self.model_service.parse_3mf_model_info(stl_path)
            self.assertFalse(model_info.has_multiple_plates)
