...
function PrinterInfoDisplay({ printerId, onMetadataFetched }: PrinterInfoDisplayProps) {
  const [printerMetadata, setPrinterMetadata] = useState<PrinterMetadata | null>(null);
      ...
      try {
        ...
        if (data.success && (data.printer_model || data.printer_name)) {
          ...
          if (configResponse.ok) {
            ...
            const currentPrinter = config.printers?.find((p: any) => p.name === printerId);
            if (currentPrinter) {
              setPrinterMetadata({
        ...
        setError(err instanceof Error ? err.message : 'Failed to fetch printer information');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const showUnknownModel = printerMetadata.printer_model === 'Unknown' || !printerMetadata.printer_model;
  const showUnknownName = printerMetadata.printer_name === 'Unknown' || !printerMetadata.printer_name;
    ...
    <div className="printer-info-display">
      ...
      <div className="info-content">
        <div className="info-grid">
          <div className="info-item">
            <span className="info-label">Model:</span>
            <span className={`info-value model ${showUnknownModel ? 'unknown' : ''}`}>
              {showUnknownModel ? 'Detecting...' : getPrinterModelDisplay(printerMetadata.printer_model)}
            </span>
          </div>
          
          <div className="info-item">
            <span className="info-label">Name:</span>
            <span className={`info-value name ${showUnknownName ? 'unknown' : ''}`}>
          ...
          <div className="info-item">
            ...
            <span className="info-value features">
              ...
              {!printerMetadata.has_serial_number && !printerMetadata.has_access_code && (
