...
test.describe('Model Preview Rendering Tests', () => {
  ...
  test('should render 3D preview for 3MF model file (Benchy)', async ({ page }) => {
    ...
    const analyzeButton = page.locator('button').filter({ hasText: /analyze|submit|process/i }).first();
    await expect(analyzeButton).toBeVisible();
    await analyzeButton.click();
    
    // Wait for model processing - should show success
    await page.waitForSelector('[data-testid="model-analysis-success"], .model-preview', {
      timeout: 30000,
    });
    ...
    await expect(modelPreview.locator('.model-preview-header')).toContainText('Model Preview');
    
    console.log('✅ 3D model preview rendered successfully for Benchy 3MF');
  });

  test('should handle model preview initialization errors gracefully', async ({ page }) => {
  ...
  test('should display model preview component structure correctly', async ({ page }) => {
    ...
    if (await modelUrlInput.isVisible()) {
      ...
      if (await analyzeButton.isVisible()) {
        ...
        if (await modelPreview.isVisible()) {
          ...
          const previewContainer = modelPreview.locator('.model-preview-container');
