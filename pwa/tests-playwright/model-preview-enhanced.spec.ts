...
test.describe('Enhanced Model Preview Tests', () => {
  ...
  test('should fallback to thumbnail when Three.js loading fails', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      window.addEventListener('load', () => {
        if (window.THREE && window.THREE.ThreeMFLoader) {
          ...
          window.THREE.ThreeMFLoader.prototype.load = function(url, onLoad, onProgress, onError) {
    ...
    if (await modelUrlInput.isVisible()) {
      ...
      if (await analyzeButton.isVisible()) {
        ...
        if (await modelPreview.isVisible()) {
          ...
          const thumbnailImage = modelPreview.locator('img[alt="Model Thumbnail"]');
          ...
          if (await thumbnailImage.isVisible()) {
            ...
            await expect(modelPreview.locator('text=📷 Thumbnail Preview')).toBeVisible();
  ...
  test('should test thumbnail API endpoint directly', async ({ page }) => {
    ...
    const response = await page.request.post(`${BACKEND_URL}/api/model/submit-url`, {
      data: {
  ...
  test('should handle multi-part 3MF files with improved geometry handling', async ({ page }) => {
    await page.goto(PWA_URL);
    await page.waitForTimeout(2000);
    ...
    if (await modelUrlInput.isVisible()) {
      ...
      if (await analyzeButton.isVisible()) {
        ...
        if (await modelPreview.isVisible()) {
          ...
          if (hasCanvas) {
            console.log('✅ Multi-part 3MF rendered with Three.js (possibly merged geometries)');
    ...
    if (await modelUrlInput.isVisible()) {
      ...
      if (await analyzeButton.isVisible()) {
        ...
        if (await modelPreview.isVisible()) {
          ...
          const multiMaterialNote = modelPreview.locator('text=Multi-material models');
          ...
          const thumbnailIndicator = modelPreview.locator('text=📷 Thumbnail Preview');
          if (await thumbnailIndicator.isVisible()) {
            console.log('✅ Thumbnail preview indicator displayed');
          }
          
          console.log('✅ Model preview component rendered with appropriate indicators');
        }
      }
    }
  });
